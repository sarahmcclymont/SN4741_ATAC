...
# Update: To normalize by read count for IGV display, can't use toTDF command, have to use `count` from the bam files. Repeat and use this for display. 
...
module load Java
for file in *.bam; do
  ~/Software/IGV_2.15.2/igvtools count $file ${file%.bam}.count.pair.tdf --pairs mm10
done
exit 0
...
# Comparing SN4741 samples to the MB and FB samples 

Use DiffBind functions to find the correlation between all of the libraries. Also show overlap of peaks with Venn diagrams/upset plots. 

Make new directory (MB_comparison) and softlink the bams and blacklisted bed files of all MB, FB, 37 and 39 libraries. 

## Correlation and PCA plots  

```{r, eval = F}

R #v4.2.2 -- for compatibility with a bunch of DiffBind dependencies, couldn't get install otherwise
...
# Full library size, RLE normalization
comparison_normalized <- dba.normalize(comparison_counts, library = "full", normalize = "RLE",  method=DBA_DESEQ2) # See temperature comparison for rationale behind norm strategy
...
# Retreiving the peaksets for each sample type for IGV visualization 
MB_consensus <- dba.peakset(venn,venn$masks$MB, bRetrieve=T)
FB_consensus <- dba.peakset(venn,venn$masks$FB, bRetrieve=T)
t37_consensus <- dba.peakset(venn,venn$masks$t37, bRetrieve=T)
t39_consensus <- dba.peakset(venn,venn$masks$t39, bRetrieve=T)

write.table(MB_consensus, "MB_consensus.peaks", quote = F, row.names=F, sep="\t")
write.table(FB_consensus, "FB_consensus.peaks", quote = F, row.names=F, sep="\t")
write.table(t37_consensus, "t37_consensus.peaks", quote = F, row.names=F, sep="\t")
write.table(t39_consensus, "t39_consensus.peaks", quote = F, row.names=F, sep="\t")

for file in *.peaks; do
  awk 'NR > 1 { print }' < $file > temp
  awk -v OFS='\t' {'print $1,$2,$3'} temp > ${file%.peaks}.bed
  done
rm temp
...
Convey the same data as the Venn diagrams, but a Venn with 4 samples can't scale the circles and overlaps correctly. An upset plot solves that but can be less intuitive to read. 

We saw from the Venn diagram that the overlap that is specific to MB/FB and 39, which is supposed to be modeling that state, is extremely low, so an upset plot might accentuate that that difference exists and is meaningful over the venn. 
...
SN4741 cells from both 37 and 39 poorly recapitulate the chromatin landscape of mouse embryonic dopaminergic neurons. 

# Comparing SN4741 samples at 37 and 39 

SN4741 cells at either temperature don't really model MB and FB samples. But there are differences between the two (some separation on the PCA plots from those initial analyses). What are those differences? Where are they in the genome? What genes are they nearby? 

Make new directory to contain the analysis and all its files ("temperature_comparison") and softlink blacklisted beds, bams, and their indices there. 

## Diffbind to find the differentially accessible regions 
...
There are clearly differences present between the chromatin state at the two temperatures (correlation heatmaps and PCA). 

Purely by peak presence and absence, there are far, far more open regions in 37. Relatively few that are specific to 39. Could be becaues 37-1 and 37-4 were sequenced so deeply, so more peaks were called in that condition, but hopefully the requirements for a peak to be present in 2 samples, and the normalization strategies negate that influence in the proper differential analysis. 

39-4 remains a weird sample, even after normalization (which "fixed" the variability present in the 37). Continue with analysis, but in light of all the QC differences and the separation on the PC plot, consider removing this sample and reanalyzing if see anything out of the ordinary. 
...
# Write out as a file
write.table(SN4741_temperature.df, "37v39_DAR_table.txt", quote = F, row.names = F)
...
dim(down39)
# 2654   12
# A few more peaks are preferentially open in 37, but for the most part, balanced.
...
# GREAT: Gene ontology of nearby genes 

Use GREAT (v4.0.4), http://bejerano.stanford.edu/great/public/html/index.php, from the Bejerano lab, to associate the peaks with the nearest gene (not the best, but an okay proxy for cognate genes). Change basal plus extension rules from 5/1kb to 5/5kb.

Also extract a list of the nearby genes for comparison with the RNA-seq. 
